...
2. **How you ran the script**: typing `.\SweepShield.ps1` in a console, `powershell -File ...`, or right-click Run with PowerShell — the same bug may only appear in ONE launch mode (see issue #1)
...
## Hard rules when editing `SweepShield.ps1`
...
- **Never use `.GetNewClosure()`.** The closure gets bound to a dynamic module and loses access to script-scope functions — the bug only shows when running `.\SweepShield.ps1` directly (issue #1). An AST test guards against it.
- **Test both launch modes**: `powershell -File SweepShield.ps1` AND typing `.\SweepShield.ps1` in a console. They place functions in different scopes (global vs script).
...
3. If behavior changes: add a `CHANGELOG.md` entry and bump `$script:SweepShieldVersion` in the script + the `VERSION` file (both must match — the self-update mechanism reads `VERSION`).
